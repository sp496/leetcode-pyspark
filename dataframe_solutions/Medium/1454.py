...
def solution_2(spark):
    ...
    result_df = l_df \
            .dropDuplicates() \
            .withColumn('previous_login', (F.col('login_date') - F.lag('login_date', 1).over(wspec))
                                .cast('int')) \
            .fillna(1, 'previous_login') \
            .groupby('id').agg(F.count(F.when(F.col('previous_login') == 1, True)).alias('consecutive_logins')) \
            .filter(F.col('consecutive_logins') >= 5) \
            .join(a_df, on='id') \
            .select('id', 'name')
